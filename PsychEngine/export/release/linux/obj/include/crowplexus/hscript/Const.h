// Generated by Haxe 4.3.7
#ifndef INCLUDED_crowplexus_hscript_Const
#define INCLUDED_crowplexus_hscript_Const

#ifndef HXCPP_H
#include <hxcpp.h>
#endif

HX_DECLARE_CLASS2(crowplexus,hscript,Const)
namespace crowplexus{
namespace hscript{


class Const_obj : public ::hx::EnumBase_obj
{
	typedef ::hx::EnumBase_obj super;
		typedef Const_obj OBJ_;

	public:
		Const_obj() {};
		HX_DO_ENUM_RTTI;
		static void __boot();
		static void __register();
		static bool __GetStatic(const ::String &inName, Dynamic &outValue, ::hx::PropertyAccess inCallProp);
		::String GetEnumName( ) const { return HX_("crowplexus.hscript.Const",7e,a7,5f,e4); }
		::String __ToString() const { return HX_("Const.",8b,b1,70,fc) + _hx_tag; }

		static ::crowplexus::hscript::Const CFloat(Float f);
		static ::Dynamic CFloat_dyn();
		static ::crowplexus::hscript::Const CInt(int v);
		static ::Dynamic CInt_dyn();
		static ::crowplexus::hscript::Const CString(::String s);
		static ::Dynamic CString_dyn();
};

} // end namespace crowplexus
} // end namespace hscript

#endif /* INCLUDED_crowplexus_hscript_Const */ 
